...
const (
	...
	socketNamePrefix   = "ch-vmm-"
	resourceNamePrefix = "devices.quill.today/"
)
...
func (dp *devicePlugin) register() error {
	...
	conn, err := grpc.NewClient(fmt.Sprintf("unix://%s", kubeletSocket), grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithBlock())
	if err != nil {
...
func waitForGRPCServer(socketPath string, timeout time.Duration) error {
	c, err := grpc.NewClient(socketPath,
